...
public class PangGridController {
	...
	private void mouseClickHandle(MouseEvent mouseEvent) {
    	...
    	if(srcLoc==null){
    		...
    		if(model.processSpecialPokemon(r,c)){ // 특수기능 실행.
    			processClick();
	...
	public void initGame(){
		Sound.stop("normal_ending");
		Sound.stop("champ_ending");
		Random bgm = new Random();
		model.initAssign();
		comboTimeIndex = 0;
		restartGameOverTimeLine(); // 타이머 시작
		restartHintTimeLine(); // 힌트 타이머 시작
		restartComboTimeLine();	// 콤보 타이머 시작
		if(bgm.nextInt(100) < 50) // 랜덤으로 배경음악 재생.
			Sound.play("game_bgm1");
		else
			Sound.play("game_bgm2");
	}
